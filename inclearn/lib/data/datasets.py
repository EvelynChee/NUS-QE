...
import inclearn.lib.data.cub200 as cub200
...
class iCUB200(DataHandler):
    ...
    def base_dataset(self, data_path, train=True, download=False):
        dataset = cub200.CUB200(data_path, train, download)
        self.data = np.array(dataset.data['filepath'])
        self.targets = np.array(dataset.data['target'])

        return self        

class iHAM10000(DataHandler):
    ...
    def base_dataset(self, data_path, train=True, download=False):
        ...
        self.data = np.array(df['image'])
        self.targets = np.argmax(np.array(df.drop(columns=['image'])), axis=1)

        return self
    
class iDiabetic(DataHandler):

    train_transforms = [
        transforms.RandomResizedCrop(512, scale=(0.8, 1.2), ratio=(0.75, 1.3333333333333333)),
        transforms.RandomAffine(degrees=180, translate=(0.05,0.05), interpolation=transforms.InterpolationMode.BILINEAR),
        transforms.RandomHorizontalFlip(),
    ]
    test_transforms = [
    ...
    common_transforms = [
        transforms.ToTensor(),
#         transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        transforms.Normalize(mean=[0.37376693, 0.37283142, 0.37173335], std=[0.24538085, 0.24495832, 0.23954594])
    ]
    ...
    def base_dataset(self, data_path, train=True, download=False):
        if train is True:
            metadata_path = os.path.join(data_path, 'trainLabels_process.csv')
        else:
            metadata_path = os.path.join(data_path, 'testLabels_process.csv')

        df = pd.read_csv(metadata_path)    
        df['image'] = df['image'].apply(lambda x: os.path.join(data_path, 'train_process' if train else 'test_process', x+'.jpeg'))
        self.data = np.array(df['image'])
        self.targets = np.array(df['level'])
